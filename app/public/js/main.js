$(document).ready(function(){
	$('#uploader').submit(function(event){
		...
		$.ajax({
			...
			processData: false,
			contentType: false,
			success: function(data, textStatus, jqXHR){if (data.status){window.open('/getcsv', '_blank'); window.open('/getzip', '_blank');$('#startWork').html('Обработать')}},
